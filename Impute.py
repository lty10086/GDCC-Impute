...
def main():
    ...
    # --- 数据与路径参数 ---
    parser.add_argument('--datasets', type=str, nargs='+', default=['Dataset1/30%', 'Dataset1/40%','Dataset1/50%','Dataset1/60%','Dataset1/70%','Dataset1/80%',
                                                                    'Klein', 'Kolodziejczyk', 'Time', 'Type', 'Zeisel'],
                        help='Datasets to process (default: Need Input)')
    ...
    parser.add_argument('--resolution', type=float, default=0.5, help='Leiden resolution (default: 0.3)')
    ...
    for dataset in args.datasets:
        ...
        if os.path.exists(labels_file):
            ...
            elif 'Label' in cellinfo.columns:
                Y = cellinfo['Label'].values
                label = Y-1
                true_label = np.array(label)
            else:
                raise KeyError("Neither 'x' nor 'Label' column found in cellinfo.")
