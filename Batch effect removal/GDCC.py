...
def preprocessing(df_drop_data):
...
def identify_dropout(leiden_labels, X, drop_data):
    ...
    def calculate_mask(cluster_cell_idxs, X):
    ...
    data_identi = calculate_mask(cluster_cell_idxs, X).T
...
def train_DCImpute(generator, discriminator, gene_data, adj_matrix, num_epochs, labels, mask, 
                   labels_sub1, labels_sub2, latent_dim, gp_weight, G_epoch, D_epoch, device=torch.device("cuda")):
